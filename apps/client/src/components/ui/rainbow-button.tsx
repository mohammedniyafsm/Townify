  ...
  ),
  {
    variants: {
      ...
      responsiveSize: {
        ...
        md: "md:h-10 md:px-6 md:text-sm",
        lg: "md:h-11 md:px-8 md:text-base",
      }
    },
...
const RainbowButton = React.forwardRef<HTMLButtonElement, RainbowButtonProps>(
  ({ className, variant, size, responsiveSize, asChild = false, ...props }, ref) => {
    const Comp = asChild ? Slot : "button"
    return (
      <Comp
        data-slot="button"
        className={cn(rainbowButtonVariants({ variant, size, responsiveSize, className }))}
        ref={ref}
        {...props}
      />
    )
